// 全站中英双语文案。修改内容只需编辑此文件。
// 证书板块的证书清单在 certs/certs.js 里维护。
window.CONTENT = {
  zh: {
    meta: { lang: "zh", title: "方海潮 · Fang (Carina) Hai-Chao" },
    nav: {
      logo: "方海潮",
      recruit: "创业",
      photos: "照片",
      education: "教育",
      experience: "经历",
      projects: "项目",
      awards: "奖项",
      certs: "证书",
      skills: "技能",
      contact: "联系",
      hobbies: "爱好"
    },
    hero: {
      name: "方海潮",
      slogan: "以静默的勇气与真挚的热爱，探索、创造，用 AI 优雅地服务于人。",
      tagline: "虔诚的基督徒，执着踏实的追梦人，可爱的小女孩",
      introLines: [
        "香港科技大学（广州）数据科学分析硕士",
        "广州大学信息与计算科学本科",
        "研究方向为大语言模型的情感智能与决策分析",
        "目前全职创业"
      ],
      linkedin: "LinkedIn",
      email: "发邮件"
    },
    recruit: {
      eyebrow: "正在招募",
      title: "我在创业，寻找同行的人",
      lead: "目前全职投入两个 AI 项目，都在寻找志同道合的队友。如果你也想用技术做一件温暖而长期的事，欢迎联系我。",
      badge: "招募队友中",
      contactBtn: "联系我",
      projects: [
        {
          name: "人生原稿",
          tagline: "用 AI 记录平凡人的不凡人生，让每一个平凡人生都被看见",
          points: [
            "五大 AI Agent 全流程自动化，生成访谈稿、自传与个人纪录片",
            "面向 3.2 亿老龄人口的银发经济与家族记忆传承需求，替代高价、低效、依赖人工的传统回忆录服务",
            "愿景：成为个人纪录片创作的专业赋能平台，建立全球最大的真实个人故事库，推动家风传承"
          ]
        },
        {
          name: "心镜 | Journey to Love",
          tagline: "为全人类创造私人定制的心灵成长之旅",
          points: [
            "面向 10-15 岁青少年的「AI 陪伴 + 游戏化成长 + SEL 情绪教育 + 家长联动」产品，切入国内空白的预防式情绪教育市场",
            "双 Agent 成长陪伴系统：受伤的 AI 宠物触发成长，成长导师启发式引导，孩子在疗愈宠物的过程中完成与自己情绪的和解",
            "以 CASEL 五大社会情感能力为框架，融合 CBT、接纳承诺疗法与叙事疗法，构建「觉察-探索-疗愈-重塑-成长」五阶段螺旋成长体系"
          ]
        }
      ]
    },
    photos: {
      eyebrow: "生活瞬间",
      title: "照片墙",
      lead: "近期旅行与生活的一些片段。"
    },
    education: {
      eyebrow: "教育背景",
      title: "求学之路",
      items: [
        {
          date: "2024.09 - 2026.08",
          org: "香港科技大学（广州）",
          role: "数据科学分析 · 硕士",
          detail:
            "主修：Auto Machine Learning（A）、Foundation of Data Science and Analytics（A-）、Project-driven Collaborative Design Thinking（A-）。"
        },
        {
          date: "2020.09 - 2024.06",
          org: "广州大学",
          role: "信息与计算科学 · 本科",
          detail:
            "GPA 3.82/4.00，加权成绩 93/100。主修：数学模型（97）、概率论与数理统计（98）、常微分方程（99）、数学建模竞赛的理论与实践（优）。打下扎实的数学与计算基础，广泛参与数学建模、创新创业与英语竞赛，本科期间发表学术论文一篇。"
        }
      ]
    },
    experience: {
      eyebrow: "实习经历",
      title: "职业实践",
      items: [
        {
          date: "2024.03 - 2024.08",
          org: "中国人寿保险股份有限公司",
          role: "业务助理",
          detail:
            "负责面试邀约、接待及基础行政工作；与客户沟通，及时把握客户需求、了解客户状况，处理并妥善解决客户疑问；学习保险金融知识，全面了解保险行业运作模式与营销状况。"
        },
        {
          date: "2023.10 - 2024.03",
          org: "科大讯飞股份有限公司",
          role: "实习教师",
          detail:
            "面向 1-4 年级学生实施科学探究课程，通过引导式实验激发学习兴趣，培养观察与协作能力。"
        },
        {
          date: "2022.09 - 2022.10",
          org: "广州信商恒国际贸易有限公司",
          role: "实习运营",
          detail: "负责视频剪辑及广告投放，联系网红达人促成推广合作。"
        },
        {
          date: "2021.06 - 2021.09",
          org: "上海高顿教育培训有限公司",
          role: "市场营销推广",
          detail:
            "配合公司制定针对本校的市场战略与营销方案，负责推广计划落地实施；提升学员转化率，通过持续跟进与服务维护良好健康的客户关系。"
        }
      ]
    },
    research: {
      eyebrow: "科研能力",
      title: "研究与技术栈",
      papers: [
        {
          title:
            "The dynamics of gene transcription induced by variation in transcription kinetics",
          venue: "发表于 Journal of Applied Analysis & Computation（本科期间）",
          link: "https://www.jaac-online.com/article/doi/10.11948/20230072"
        },
        {
          title:
            "Evaluating LLM Decision-Making in Daily Dilemmas: Effects of Emotional Framing, Analytical Reasoning, and Model Architecture",
          venue: "EMNLP 在审"
        }
      ],
      tags: [
        "大语言模型 (LLM)",
        "情感智能",
        "决策行为分析",
        "提示工程",
        "推理机制评估",
        "Python",
        "SQL",
        "数据工程",
        "统计分析与实验设计",
        "NLP 文本分类 / 情感分析",
        "数据清洗 / 特征工程",
        "写意编程"
      ]
    },
    projects: {
      eyebrow: "项目经历",
      title: "项目经历",
      studyTitle: "研学",
      contestTitle: "竞赛",
      study: [
        {
          name: "新加坡沉浸式研学",
          role: "组长 · 2025.12",
          detail:
            "基于华为云的沉浸式沙盒环境构建聊天机器人；以新加坡语境为背景，通过以学生为中心的故事角色扮演激发中文学习动机；走访 Singtel 企业及当地创业机构，探索新加坡饮食、历史、城市景观与人文风貌。"
        },
        {
          name: "法国超文化人类学研学",
          role: "成员 · 2025.05 - 2025.06",
          detail:
            "深度探访法国（尤其是巴黎）文化地标，理解法国文化遗产；调研当地习俗与实践，培养跨文化认知与适应能力；反思人类共通价值，发展超文化视角与全球责任感。"
        }
      ],
      contest: [
        {
          name: "知行研学",
          role: "队员 · 2021.09 - 2022.06",
          detail:
            "获 2021 年青蓝国际创新创业大赛优胜奖、第十三届挑战杯省赛铜奖。负责课程编写制作、计划书与 PPT 制作、研学活动的组织，以及比赛路演答辩。"
        },
        {
          name: "香港会计师公会商业个案比赛",
          role: "队长 · 2021.08 - 2021.11",
          detail:
            "组织团队成员开展赛事内容研讨，统筹项目日程规划，高效推进报告撰写与执行；主导大量资料检索与行业专业人士调研，对团队想法进行逻辑提炼与总结升华。"
        },
        {
          name: "白金元宇宙",
          role: "队员 · 2022.07 - 2022.11",
          detail:
            "获第八届中国国际「互联网+」大学生创新创业大赛全国总决赛银奖。参与项目整体规划与推进，系统学习视频剪辑、PPT 高级美化及商业计划书撰写；独立完成多项视觉设计与内容优化，借助赛事平台与领域资深专家交流，拓展行业认知。"
        },
        {
          name: "新点科技 · 智能作文批改",
          role: "队长 · 2021.02 - 2023.09",
          detail:
            "获第十二届「创新创意创业」比赛省级二等奖；申请「批改作文的方法、系统及装置」国家发明专利一项（已进入实审阶段），AI 智能批改作文软件著作权一项。项目致力于让科技推动教育发展，创新性构思集软硬件为一体的智能作文批改机器。"
        },
        {
          name: "乘风出海东南亚",
          role: "队长 · 2022.09",
          detail:
            "获第十三届全国大学生电子商务「创新、创意及创业」挑战赛省级一等奖。深入调研跨境电商行业运营模式与市场环境，把握行业发展趋势；统筹项目整体推进，组织团队完成商业计划书与 PPT 并持续迭代优化。"
        }
      ]
    },
    awards: {
      eyebrow: "奖项荣誉",
      title: "荣誉墙",
      groups: [
        {
          group: "数学建模竞赛",
          items: [
            "2022 年亚太地区大学生数学建模竞赛（APMCM）国际二等奖",
            "2022 年数维杯国际二等奖",
            "美国大学生数学建模竞赛（MCM/ICM）H 奖"
          ]
        },
        {
          group: "商业与数据分析竞赛",
          items: [
            "第八届中国国际「互联网+」大学生创新创业大赛全国总决赛银奖",
            "2023 年正大杯市场调查分析比赛省级一等奖（统筹管理团队，综合运用统计与数学知识完成研究报告）",
            "意大利国际创业竞赛三等奖"
          ]
        },
        {
          group: "英语竞赛",
          items: [
            "2023 年全国大学生英语竞赛（NECCS）一等奖",
            "2022 年「外研社·国才杯」全国英语阅读比赛国赛三等奖",
            "2022 年「外研社·国才杯」全国英语写作比赛校赛一等奖",
            "第 24 届「外研社·国才杯」全国英语辩论比赛校赛一等奖",
            "首届「外教杯·词达人杯」全国大学生英语词汇能力大赛国赛三等奖"
          ]
        }
      ]
    },
    certs: {
      eyebrow: "资质认证",
      title: "证书",
      lead: "一些获得的证书与资质。点击图片可放大查看。",
      viewBtn: "查看证书"
    },
    campus: {
      eyebrow: "校园与社会",
      title: "校园与社会经历",
      campusTitle: "校园经历",
      societyTitle: "社会经历",
      campus: [
        {
          name: "红鸟国际交流项目（RISE Week）",
          role: "教学助理 · 2026.01",
          detail: "面向乌兹别克斯坦学生开展跨文化项目制学习（PBL）辅导，促进跨文化协作，建立有意义的教育连接。"
        },
        {
          name: "校学生艺术团行政部门办公室",
          role: "主要负责人 · 2021.09 - 2022.07",
          detail:
            "撰写策划书、设置工作分组、组织工作人员有序开展活动，全方位把控会议进程；具备应对紧急事件的能力，能在各项活动中注入独到想法、增添亮点。"
        },
        {
          name: "辩论队",
          role: "成员 · 2020.09 - 2022.08",
          detail: "2020 年代表班级参加学院「新生杯」中文辩论比赛并获得冠军；广州大学英语辩论队成员，曾获 Best Debater 称号。"
        },
        {
          name: "学院部门大会",
          role: "主持人 · 2020.09 - 2021.06",
          detail: "撰写大会文稿并全程主持，成功把控活动流程、调动参与者积极性，获得积极的评价与反馈。"
        },
        {
          name: "校舞蹈队",
          role: "成员 · 2020.09 - 2021.02",
          detail: "参与广州大学合并组建 20 周年迎新晚会及成果展演出。"
        }
      ],
      society: [
        {
          name: "私人教师",
          role: "2020 至今",
          detail:
            "辅导小学至高中学生英语与数学，建立良好师生关系并提供长期学业与生活指导；耐心引导并与学生做朋友，激发兴趣、促进自主学习。",
          bullets: [
            "高三学生：40～50 分 → 高考 96 分",
            "初三学生：70～90 分 → 106 分",
            "一对一及一对多经验丰富",
            "雅思口语培训 / 雅思英语小班教学"
          ]
        },
        {
          name: "志愿者 · 笃学助飞韶关志教",
          role: "2026.07.11 - 2026.07.21",
          detail:
            "在韶关乐昌市坪梅中学担任笃学助飞工作室英语戏剧组小老师，带领的小组获得最佳成果奖。结合个人超文化项目实践学习，港科广跨学科融合教育，为学生带来了愉快又前沿的学习方式。结束时，我收到了孩子们的一封封信件，表达他们从自卑到自信，害怕到勇敢这一程转变生出的感恩之心。",
          images: [
            { src: "photos/volunteer/duxue-shaoguan-web.jpg", alt: "笃学助飞韶关志教" },
            { src: "photos/volunteer/first-place-web.jpg", alt: "第一名" },
            { src: "photos/volunteer/best-outcome-award-web.jpg", alt: "最佳成果奖" }
          ],
          links: [
            { label: "微信报道一", href: "https://mp.weixin.qq.com/s/EFEO93-FGec3TDTFTzW4hQ" },
            { label: "微信报道二", href: "https://mp.weixin.qq.com/s/YhAy-v_soygl-s4B5_LEDQ" }
          ]
        },
        {
          name: "志愿者",
          role: "2021 至今",
          detail:
            "走访经济困难家庭，为老人贴对联、打扫卫生；参与防疫志愿服务、担任音乐会后台工作人员等。完成了为期十周的「童年一课」伴读志愿活动，并获得「匠心伴读」「分享之星」荣誉证书。",
          images: [
            { src: "photos/volunteer/童年一课志愿服务证书-web.jpg", alt: "童年一课志愿服务证书" },
            { src: "photos/volunteer/童年一课匠心伴读-web.jpg", alt: "童年一课匠心伴读" },
            { src: "photos/volunteer/分享之星-web.jpg", alt: "分享之星" }
          ]
        },
        {
          name: "💎 旅行爱好者",
          role: "国内外独自旅行",
          detail:
            "足迹遍及武汉、上海、北京、杭州、巴黎、罗马、佛罗伦萨、梵蒂冈等地，具备共情理解与跨文化沟通、国际交流能力。"
        }
      ]
    },
    skills: {
      eyebrow: "技能特长",
      title: "能力与专长",
      items: [
        {
          name: "编程与技术",
          detail:
            "Python（主要）、C、C++、Java；掌握 HTML、CSS、Java Web 开发基础；具备 SQL 数据查询、分析与数据库应用能力；熟练使用 PowerPoint、Excel、Word 等办公软件，熟悉 AI 工具与大模型应用。"
        },
        {
          name: "语言能力",
          detail: "英语流利：雅思 8.0，国际人才英语考试（高级）；意大利语初级。"
        },
        {
          name: "金融专业",
          detail: "通过金融风险管理师（FRM）一级与二级考试。"
        },
        {
          name: "公众演讲",
          detail:
            "三分钟英语论文演讲总决赛入围选手，多次获得演讲比赛冠亚军。"
        },
        {
          name: "心理咨询",
          detail:
            "完成岭南大学与和富李祖尧辅导与心理研究中心举办的心理健康大使项目。"
        },
        {
          name: "核心能力",
          detail:
            "成长型思维，爱思考善学习，创新想法层出不穷；具备出色的分析能力，善于发现问题并应对复杂情境；擅长将复杂问题分解为简单要素，运用第一性原理提出有效解决方案；执行力强、坚持不懈，具备跨学科综合思维与适应能力。"
        }
      ]
    },
    contact: {
      eyebrow: "联系方式",
      title: "联系我",
      lead: "无论是创业合作、项目交流还是单纯想认识，都欢迎联系。",
      items: [
        { label: "邮箱", value: "fancyfhc@163.com", href: "mailto:fancyfhc@163.com" },
        { label: "电话", value: "+86 190-6533-1466", href: "tel:+8619065331466" },
        {
          label: "LinkedIn",
          value: "hai-chao-fang",
          href: "https://www.linkedin.com/in/hai-chao-fang-00b622287"
        }
      ]
    },
    hobbies: {
      eyebrow: "生活趣味",
      title: "爱好",
      lead: "工作之外，也认真地爱着这些小事。",
      items: [
        { emoji: "📚", label: "看书" },
        { emoji: "✍️", label: "写作" },
        { emoji: "🎤", label: "唱歌" },
        { emoji: "💃", label: "跳舞" },
        { emoji: "🎬", label: "看电影" },
        { emoji: "🏛️", label: "逛博物馆" },
        { emoji: "🎨", label: "赏画展" }
      ]
    },
    footer: "© 2026 方海潮 Fang (Carina) Hai-Chao · 用心打造"
  },

  en: {
    meta: { lang: "en", title: "Fang (Carina) Hai-Chao" },
    nav: {
      logo: "Fang Hai-Chao",
      recruit: "Building",
      photos: "Photos",
      education: "Education",
      experience: "Experience",
      projects: "Projects",
      awards: "Awards",
      certs: "Certificates",
      skills: "Skills",
      contact: "Contact",
      hobbies: "Hobbies"
    },
    hero: {
      name: "Fang (Carina) Hai-Chao",
      slogan: "With quiet courage and heartfelt love, I explore, create, and serve people gracefully with AI.",
      tagline: "A devout Christian, a steadfast and grounded dream-chaser, a lovely girl",
      introLines: [
        "MPhil in Data Science and Analytics, HKUST (Guangzhou)",
        "BSc in Information and Computing Science, Guangzhou University",
        "Research focus: affective intelligence and decision-making in LLMs",
        "Now building full-time as a founder"
      ],
      linkedin: "LinkedIn",
      email: "Email me"
    },
    recruit: {
      eyebrow: "Now Recruiting",
      title: "I'm Building — Join Me",
      lead: "I'm working full-time on two AI products, and I'm looking for teammates for both. If you want to build something warm and long-lasting with technology, let's talk.",
      badge: "Looking for teammates",
      contactBtn: "Get in touch",
      projects: [
        {
          name: "Life Manuscript",
          tagline: "Using AI to record the extraordinary lives of ordinary people — so every life is seen",
          points: [
            "Five AI agents automate the full pipeline, generating interview transcripts, autobiographies, and personal documentaries",
            "Serving the silver economy of 320 million seniors and family-memory preservation, replacing costly, slow, labor-intensive memoir services",
            "Vision: become the professional enablement platform for personal documentaries and build the world's largest library of authentic life stories"
          ]
        },
        {
          name: "Mind Mirror | Journey to Love",
          tagline: "A personalized journey of inner growth, for everyone",
          points: [
            "For ages 10-15: 'AI companionship + gamified growth + SEL emotional education + parent involvement', entering a blue-ocean market for preventive emotional education in China",
            "A dual-agent system: a wounded AI pet triggers growth while a mentor guides through inspiration — children reconcile with their own emotions by healing the pet",
            "Built on the five CASEL competencies, integrating CBT, Acceptance & Commitment Therapy and narrative therapy into a five-stage spiral growth framework: awareness, exploration, healing, reshaping, growth"
          ]
        }
      ]
    },
    photos: {
      eyebrow: "Moments",
      title: "Photo Wall",
      lead: "A few recent moments from travel and life."
    },
    education: {
      eyebrow: "Education",
      title: "Academic Journey",
      items: [
        {
          date: "Sep 2024 - Aug 2026",
          org: "HKUST (Guangzhou)",
          role: "MPhil, Data Science and Analytics",
          detail:
            "Highlights: Auto ML (A), Foundation of Data Science and Analytics (A-), Project-driven Collaborative Design Thinking (A-)."
        },
        {
          date: "Sep 2020 - Jun 2024",
          org: "Guangzhou University",
          role: "BSc, Information and Computing Science",
          detail:
            "GPA 3.82/4.00, weighted 93/100. Highlights: Mathematical Model (97), Probability and Mathematical Statistics (98), ODE (99), Mathematical Modeling Contest Theory and Practice (Excellent). Built a solid foundation in mathematics and computing; active in modeling, entrepreneurship and English competitions, with one academic paper published as an undergraduate."
        }
      ]
    },
    experience: {
      eyebrow: "Internships",
      title: "Professional Experience",
      items: [
        {
          date: "Mar 2024 - Aug 2024",
          org: "China Life Insurance Co., Ltd.",
          role: "Business Assistant",
          detail:
            "Handled interview invitations, reception and administrative work; communicated with clients to grasp needs, understand their situations and properly resolve questions; studied insurance and finance to understand the industry's operating and marketing model."
        },
        {
          date: "Oct 2023 - Mar 2024",
          org: "iFlytek Co., Ltd.",
          role: "Teacher",
          detail:
            "Taught engaging, experiment-based science classes for Grades 1-4, fostering curiosity and collaborative discovery."
        },
        {
          date: "Sep 2022 - Oct 2022",
          org: "Guangzhou Xinshangheng Int'l Trade Co., Ltd.",
          role: "Operations Intern",
          detail:
            "Responsible for video editing and advertising, and contacting influencers to establish promotion partnerships."
        },
        {
          date: "Jun 2021 - Sep 2021",
          org: "Shanghai Gordon Education & Training Co., Ltd.",
          role: "Marketing",
          detail:
            "Developed and executed the school-targeted marketing strategy and plan; improved student conversion and maintained healthy client relationships through continuous follow-up and service."
        }
      ]
    },
    research: {
      eyebrow: "Research",
      title: "Research & Technical Stack",
      papers: [
        {
          title:
            "The dynamics of gene transcription induced by variation in transcription kinetics",
          venue: "Published in Journal of Applied Analysis & Computation (undergrad)",
          link: "https://www.jaac-online.com/article/doi/10.11948/20230072"
        },
        {
          title:
            "Evaluating LLM Decision-Making in Daily Dilemmas: Effects of Emotional Framing, Analytical Reasoning, and Model Architecture",
          venue: "Under review at EMNLP"
        }
      ],
      tags: [
        "Large Language Models",
        "Affective Intelligence",
        "Decision-Making Analysis",
        "Prompt Engineering",
        "Reasoning Evaluation",
        "Python",
        "SQL",
        "Data Engineering",
        "Statistics & Experiment Design",
        "NLP: Classification / Sentiment",
        "Data Cleaning / Feature Engineering",
        "Vibe Coding"
      ]
    },
    projects: {
      eyebrow: "Projects",
      title: "Projects",
      studyTitle: "Study Trips",
      contestTitle: "Competitions",
      study: [
        {
          name: "Immersive Study Program, Singapore",
          role: "Leader · Dec 2025",
          detail:
            "Built a chatbot in an immersive sandbox on HUAWEI Cloud; cultivated motivation for Chinese learning through student-centered story role-play in a Singapore context; visited Singtel and local startup institutions while exploring Singapore's food, history, landscapes and people."
        },
        {
          name: "France Transcultural Anthropology Study",
          role: "Member · Apr - Jun 2025",
          detail:
            "Explored France's cultural landmarks (especially Paris) to understand cultural heritage; researched local customs to strengthen cross-cultural competence; reflected on universal human values to cultivate a transcultural perspective and global responsibility."
        }
      ],
      contest: [
        {
          name: "Zhixing Study Program",
          role: "Member · Sep 2021 - Jun 2022",
          detail:
            "Winner's Award, 2021 Qinglan International Innovation & Entrepreneurship Competition; Provincial Bronze, 13th Challenge Cup. Responsible for course design and production, proposals and slides, organizing study activities, and the contest roadshow defense."
        },
        {
          name: "HKICPA Business Case Competition",
          role: "Captain · Aug - Nov 2021",
          detail:
            "Organized team research discussions and overall scheduling to drive the report forward efficiently; led extensive research and interviews with industry professionals, refining and elevating the team's ideas with patience and rigor."
        },
        {
          name: "Platinum Universe",
          role: "Member · Jul - Nov 2022",
          detail:
            "National Finals Silver Award, 8th China International 'Internet+' Innovation & Entrepreneurship Competition. Contributed to overall planning; learned video editing, advanced slide design and business-plan writing; independently delivered visual design and content optimization while connecting with senior industry experts."
        },
        {
          name: "Xindian Tech · AI Essay Grading",
          role: "Captain · Feb 2021 - Sep 2023",
          detail:
            "Provincial Second Prize, 12th 'Innovation, Creativity and Entrepreneurship' Challenge; filed one national invention patent for an essay-correction method, system and device (now in substantive examination) and one software copyright for AI essay grading. Conceived an integrated hardware-software intelligent essay-grading machine to advance education through technology."
        },
        {
          name: "Sail to Southeast Asia",
          role: "Captain · Sep 2022",
          detail:
            "Provincial First Prize, 13th National College Student E-Commerce 'Innovation, Creativity and Entrepreneurship' Challenge. Researched cross-border e-commerce operations and market conditions; coordinated the whole project and led the team on the business plan and slides with iterative optimization."
        }
      ]
    },
    awards: {
      eyebrow: "Honors",
      title: "Awards & Honors",
      groups: [
        {
          group: "Mathematical Modeling",
          items: [
            "Second Prize (International), Asia-Pacific Mathematical Contest in Modeling (APMCM), 2022",
            "Second Prize (International), MathorCup Mathematical Modeling Challenge, 2022",
            "Honorable Mention (H Award), MCM/ICM"
          ]
        },
        {
          group: "Business & Data Analytics",
          items: [
            "National Finals Silver Award, 8th China International 'Internet+' Innovation & Entrepreneurship Competition",
            "First Prize (Provincial), CP Group Market Research and Analysis Competition, 2023 — led project coordination and team management, applying statistical and quantitative methods to the final report",
            "Third Prize, International Entrepreneurship Competition (Italy)"
          ]
        },
        {
          group: "English Competitions",
          items: [
            "First Prize (National), National English Competition for College Students (NECCS), 2023",
            "Third Prize (National Final), FLTRP Cup National English Reading Contest, 2022",
            "First Prize (School Final), FLTRP Cup National English Writing Contest, 2022",
            "First Prize (School Final), 24th FLTRP Cup National English Debating Contest",
            "Third Prize (National Final), 'Cidaren Cup' National English Vocabulary Competition"
          ]
        }
      ]
    },
    certs: {
      eyebrow: "Credentials",
      title: "Certificates",
      lead: "A selection of certificates and credentials. Click an image to enlarge.",
      viewBtn: "View Certificate"
    },
    campus: {
      eyebrow: "Campus & Society",
      title: "Campus & Community",
      campusTitle: "Campus",
      societyTitle: "Society",
      campus: [
        {
          name: "RISE Week Int'l Exchange",
          role: "Teaching Assistant · Jan 2026",
          detail:
            "Facilitated project-based learning for students from Uzbekistan, fostering cross-cultural collaboration and building meaningful educational connections."
        },
        {
          name: "Student Art Troupe Admin Office",
          role: "Chief · Sep 2021 - Jul 2022",
          detail:
            "Wrote planning documents, organized staff and directed events end-to-end with full control of proceedings; handled emergencies and injected unique ideas to add highlights to every event."
        },
        {
          name: "Debate Team",
          role: "Debater · Sep 2020 - Aug 2022",
          detail:
            "University Freshmen Cup champion (Chinese debate, 2020) and 'Best Debater' on the Guangzhou University English Debate Team."
        },
        {
          name: "College Department Conference",
          role: "Host · Sep 2020 - Jun 2021",
          detail:
            "Authored the conference manuscript and managed all proceedings, successfully directing event flow and fostering participant engagement to positive feedback."
        },
        {
          name: "School Dancing Team",
          role: "Dancer · Sep 2020 - Feb 2021",
          detail:
            "Performed in the welcome party for the 20th anniversary of Guangzhou University's merger and the achievement exhibition."
        }
      ],
      society: [
        {
          name: "Private Tutor",
          role: "Since 2020",
          detail:
            "Tutored primary to high-school students in English and math, building lasting teacher-student relationships and providing long-term academic and life guidance; patiently guided students as friends, sparked interest, and fostered independent learning.",
          bullets: [
            "Grade 12: 40-50 → Gaokao 96",
            "Grade 9: 70-90 → 106",
            "Strong one-on-one and one-to-many teaching experience",
            "IELTS speaking / small-group IELTS English training"
          ]
        },
        {
          name: "Volunteer · Duxue Zhufei Teaching in Shaoguan",
          role: "Jul 11 - Jul 21, 2026",
          detail:
            "Served as a junior teacher for the English drama group of Duxue Zhufei Studio at Pingmei Middle School in Lechang, Shaoguan. The group I led won the Best Outcome Award. Drawing on intercultural project practice and HKUST(GZ)'s interdisciplinary education, I brought students an enjoyable and forward-looking way of learning. At the end, I received letters from the children expressing gratitude for their journey from self-doubt to confidence, and from fear to courage.",
          images: [
            { src: "photos/volunteer/duxue-shaoguan-web.jpg", alt: "Duxue Zhufei teaching in Shaoguan" },
            { src: "photos/volunteer/first-place-web.jpg", alt: "First place" },
            { src: "photos/volunteer/best-outcome-award-web.jpg", alt: "Best Outcome Award" }
          ],
          links: [
            { label: "WeChat article 1", href: "https://mp.weixin.qq.com/s/EFEO93-FGec3TDTFTzW4hQ" },
            { label: "WeChat article 2", href: "https://mp.weixin.qq.com/s/YhAy-v_soygl-s4B5_LEDQ" }
          ]
        },
        {
          name: "Volunteer",
          role: "Since 2021",
          detail:
            "Visited families in need, put up couplets and cleaned for the elderly; joined epidemic-prevention volunteering and served as backstage concert staff. Completed a ten-week 'Childhood Lesson' companion-reading volunteering program and received the Craft Companion Reader and Sharing Star certificates.",
          images: [
            { src: "photos/volunteer/童年一课志愿服务证书-web.jpg", alt: "Childhood Lesson volunteering certificate" },
            { src: "photos/volunteer/童年一课匠心伴读-web.jpg", alt: "Craft Companion Reader certificate" },
            { src: "photos/volunteer/分享之星-web.jpg", alt: "Sharing Star certificate" }
          ]
        },
        {
          name: "💎 Travel Enthusiast",
          role: "Solo trips at home and abroad",
          detail:
            "Traveled to Wuhan, Shanghai, Beijing, Hangzhou, Paris, Rome, Florence, Vatican and more — empathetic, personable and capable of international and cross-cultural communication."
        }
      ]
    },
    skills: {
      eyebrow: "Skills",
      title: "Skills & Expertise",
      items: [
        {
          name: "Programming & Tech",
          detail:
            "Python (primary), C, C++, Java; HTML, CSS and Java Web fundamentals; SQL querying, analysis and database applications; proficient in PowerPoint, Excel and Word, experienced with AI-assisted workflows and LLM applications."
        },
        {
          name: "Languages",
          detail:
            "Fluent English — IELTS 8.0, English Test for International Communication (Advanced); elementary Italian."
        },
        {
          name: "Finance",
          detail: "Passed Financial Risk Manager (FRM) Levels I & II."
        },
        {
          name: "Public Speaking",
          detail:
            "Finalist, Three Minute Thesis Final Competition, with multiple champion and runner-up finishes in speech contests."
        },
        {
          name: "Counseling Psychology",
          detail:
            "Completed the Wellness Ambassador Programme (Lingnan University & Wofoo Joseph Lee Consulting and Counseling Psychology Research Center)."
        },
        {
          name: "Core Competencies",
          detail:
            "Creative thinker with a growth mindset, constantly learning and generating innovative ideas; strong analytical skills for complex scenarios; breaks problems into simple components with first-principles thinking; proven execution with persistence, adaptability and cross-disciplinary insight."
        }
      ]
    },
    contact: {
      eyebrow: "Contact",
      title: "Get in Touch",
      lead: "Whether it's about founding together, project talk, or just saying hi — reach out.",
      items: [
        { label: "Email", value: "fancyfhc@163.com", href: "mailto:fancyfhc@163.com" },
        { label: "Phone", value: "+86 190-6533-1466", href: "tel:+8619065331466" },
        {
          label: "LinkedIn",
          value: "hai-chao-fang",
          href: "https://www.linkedin.com/in/hai-chao-fang-00b622287"
        }
      ]
    },
    hobbies: {
      eyebrow: "Life & Joy",
      title: "Hobbies",
      lead: "Outside of work, I cherish these little loves too.",
      items: [
        { emoji: "📚", label: "Reading" },
        { emoji: "✍️", label: "Writing" },
        { emoji: "🎤", label: "Singing" },
        { emoji: "💃", label: "Dancing" },
        { emoji: "🎬", label: "Movies" },
        { emoji: "🏛️", label: "Museums" },
        { emoji: "🎨", label: "Art Exhibitions" }
      ]
    },
    footer: "© 2026 Fang (Carina) Hai-Chao · Made with care"
  }
};

window.LINKEDIN_URL = "https://www.linkedin.com/in/hai-chao-fang-00b622287";
